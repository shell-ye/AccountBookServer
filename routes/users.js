...
router.post('/register', async (req, res) => {
	let { username, head_img, openid, session_key  } = req.body
	username = username.replace(/\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g, "")
	if ( !username || !head_img || !openid || !session_key ) {
...
router.get('/login', middle.setLastMonth, middle.checkUser, async (req, res) => {
	...
	Utils.getUserInfo( openid ).then(data => {
		if ( data ) {
			res.send({code: 200, login: true, data})
		} else {
